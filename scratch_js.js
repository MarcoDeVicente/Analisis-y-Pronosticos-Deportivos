                        ...
                        document.addEventListener("DOMContentLoaded", () => {
                            ...
                            leagues.forEach(league => {
                                ...
                                card.innerHTML = `
                                    ...
                                    <input type="file" id="csv-input-${league.id}" accept=".csv" class="hidden" />
                                    <label for="csv-input-${league.id}" class="cursor-pointer text-[10px] bg-slate-800 text-slate-300 py-1.5 px-2 rounded border border-slate-700 hover:bg-slate-700 hover:text-white transition text-center mb-1.5 truncate shadow-sm" id="csv-label-${league.id}">
                                        Examinar...
                                    </label>
                                    <button type="button" onclick="uploadSpecificCsv('${league.id}')" id="btn-upload-${league.id}" class="mt-auto text-[10px] bg-emerald-600/20 border border-emerald-500/30 text-emerald-400 py-1.5 px-2 rounded font-bold hover:bg-emerald-600/40 transition flex justify-center items-center w-full disabled:opacity-50 disabled:cursor-not-allowed">
                                        <span>Subir</span>
                    ...
                    data.standings.forEach(row => {
                        ...
                        html += `
                            <tr class="hover:bg-slate-800/20 transition-colors" style="${bgStyle}">
                                ...
                                <td class="py-3 px-3 text-center font-extrabold text-blue-400">${row.Puntos}</td>
                                <td class="py-3 px-3 text-center">${row.Jugados}</td>
                                <td class="py-3 px-3 text-center text-emerald-400">${row.Ganados}</td>
                                <td class="py-3 px-3 text-center text-slate-400">${row.Empatados}</td>
                                <td class="py-3 px-3 text-center text-rose-400">${row.Perdidos}</td>
                                <td class="py-3 px-3 text-center text-slate-400">${row.Goles_Favor}</td>
                                <td class="py-3 px-3 text-center text-slate-400">${row.Goles_Contra}</td>
        ...
        function renderCornersPlaceholder() {
            ...
            for (let i = 0; i < 11; i++) {
                ...
                row.innerHTML = `
                    ...
                    <div class="col-span-2 text-center text-slate-500 font-bold bg-slate-900 border border-slate-800 rounded-md py-0.5 text-[10px]">
                        ${label}
                    </div>
        ...
        .sport-toggle { display: flex; background: rgba(15, 23, 42, .8); border: 1px solid rgba(255, 255, 255, .08); border-radius: 10px; padding: 3px; gap: 3px; }
        .toggle-btn { flex: 1; padding: 8px 12px; border-radius: 8px; font-size: 12px; font-weight: 600; border: none; cursor: pointer; background: transparent; color: #64748b; transition: all .2s; }
        .toggle-btn.active { background: rgba(59, 130, 246, .2); color: #fff; border: 1px solid rgba(59, 130, 246, .3); }
        .toggle-btn.active-beisbol { background: rgba(124, 58, 237, .2); color: #c4b5fd; border: 1px solid rgba(124, 58, 237, .35); box-shadow: 0 0 12px rgba(124, 58, 237, .2); }
        .selects-row { display: flex; align-items: center; gap: 8px; }
        .select-wrap { position: relative; }
        .select-wrap select { width: 100%; background: rgba(15, 23, 42, .8); border: 1px solid rgba(255, 255, 255, .08); border-radius: 10px; padding: 9px 28px 9px 12px; font-size: 12px; font-family: 'Outfit', sans-serif; color: #e2e8f0; appearance: none; cursor: pointer; }
        .select-wrap select:focus { outline: none; border-color: rgba(99, 102, 241, .5); }
        .select-wrap::after { content: '▾'; position: absolute; right: 10px; top: 50%; transform: translateY(-50%); font-size: 11px; color: #475569; pointer-events: none; }
        .vs-badge { font-size: 11px; font-weight: 600; color: #475569; flex-shrink: 0; padding: 0 2px; }
        .vs { font-size: 11px; font-weight: 700; color: #334155; flex-shrink: 0; }
        .winner-row { display: flex; gap: 8px; }
        .winner-btn { flex: 1; padding: 11px 8px; border-radius: 10px; border: 1px solid rgba(255, 255, 255, .08); background: rgba(30, 41, 59, .3); color: #64748b; font-size: 13px; font-weight: 600; font-family: 'Outfit', sans-serif; cursor: pointer; transition: all .2s; text-align: center; }
        .winner-btn:hover { border-color: rgba(99, 102, 241, .4); color: #e2e8f0; background: rgba(99, 102, 241, .12); }
        .winner-btn.selected-local { border-color: rgba(59, 130, 246, .6); background: rgba(59, 130, 246, .18); color: #93c5fd; box-shadow: 0 0 12px rgba(59, 130, 246, .2); }
        .winner-btn.selected-draw { border-color: rgba(234, 179, 8, .5); background: rgba(234, 179, 8, .12); color: #fcd34d; box-shadow: 0 0 12px rgba(234, 179, 8, .15); }
        .winner-btn.selected-away { border-color: rgba(239, 68, 68, .5); background: rgba(239, 68, 68, .12); color: #fca5a5; box-shadow: 0 0 12px rgba(239, 68, 68, .15); }
        .winner-btn.sel-home { border-color: rgba(124, 58, 237, .7); background: rgba(124, 58, 237, .2); color: #c4b5fd; box-shadow: 0 0 16px rgba(124, 58, 237, .25); }
        .winner-btn.sel-away { border-color: rgba(219, 39, 119, .6); background: rgba(219, 39, 119, .18); color: #f9a8d4; box-shadow: 0 0 16px rgba(219, 39, 119, .2); }
        .markets-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
        .market-card { background: rgba(30, 41, 59, .3); border: 1px solid rgba(255, 255, 255, .06); border-radius: 12px; padding: 14px; cursor: pointer; transition: all .2s; }
        .market-title { font-size: 10px; font-weight: 600; letter-spacing: 1px; color: #475569; margin-bottom: 8px; }
        .market-select { width: 100%; background: rgba(9, 13, 22, .6); border: 1px solid rgba(255, 255, 255, .08); border-radius: 8px; padding: 8px 10px; font-size: 12px; font-family: 'Outfit', sans-serif; color: #cbd5e1; appearance: none; cursor: pointer; }
        .market-select:focus { outline: none; border-color: rgba(16, 185, 129, .4); }
        .add-btn { width: 100%; padding: 13px; border-radius: 12px; background: linear-gradient(to right, #059669, #0d9488); border: none; color: #fff; font-size: 14px; font-weight: 700; font-family: 'Outfit', sans-serif; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 8px; transition: all .2s; letter-spacing: .5px; }
        .add-btn:hover { box-shadow: 0 0 20px rgba(16, 185, 129, .35); filter: brightness(1.1); }
        .add-btn.add-btn-beisbol { background: linear-gradient(to right, #7c3aed, #6d28d9); }
        .add-btn.add-btn-beisbol:hover { box-shadow: 0 0 22px rgba(124, 58, 237, .4); }
        .right-panel { flex: 0 0 40%; background: rgba(2, 6, 23, .7); border: 1px solid rgba(255, 255, 255, .07); border-radius: 16px; backdrop-filter: blur(16px); display: flex; flex-direction: column; overflow: hidden; }
        .ticket-header { padding: 18px 20px 14px; border-bottom: 1px solid rgba(255, 255, 255, .06); }
        .ticket-title-row { display: flex; align-items: center; gap: 8px; margin-bottom: 2px; }
        .ticket-title { font-size: 13px; font-weight: 800; letter-spacing: 2px; color: #fff; }
        .lightning { font-size: 16px; }
        .ticket-sub { font-size: 10px; color: #475569; letter-spacing: 1px; font-weight: 500; }
        .picks-list { flex: 1; overflow-y: auto; padding: 14px 16px; display: flex; flex-direction: column; gap: 8px; }
        .pick-card { display: flex; align-items: center; gap: 0; border-radius: 10px; overflow: hidden; border: 1px solid rgba(255, 255, 255, .06); background: rgba(15, 23, 42, .5); animation: slideInCard 0.25s cubic-bezier(0.16, 1, 0.3, 1); }
        .pick-stripe { width: 4px; flex-shrink: 0; align-self: stretch; }
        .pick-stripe.futbol { background: linear-gradient(to bottom, #3b82f6, #6366f1); }
        .pick-stripe.beisbol { background: linear-gradient(to bottom, #7c3aed, #db2777); }
        .pick-body { flex: 1; padding: 10px 12px; }
        .pick-sport { font-size: 10px; font-weight: 600; color: #475569; letter-spacing: 1px; margin-bottom: 2px; }
        .pick-match { font-size: 12px; font-weight: 700; color: #e2e8f0; }
        .pick-market { font-size: 11px; color: #64748b; margin-top: 1px; }
        .pick-prob { font-size: 11px; font-weight: 600; margin-top: 3px; }
        .pick-del { background: transparent; border: none; color: #334155; cursor: pointer; padding: 10px 14px; font-size: 16px; transition: color .15s; flex-shrink: 0; }
        .pick-del:hover { color: #ef4444; }
        .ticket-total { padding: 16px 20px; border-top: 2px solid rgba(255, 255, 255, .08); }
        .total-label { font-size: 10px; font-weight: 600; letter-spacing: 2px; color: #475569; margin-bottom: 6px; }
        .total-value { font-size: 44px; font-weight: 900; line-height: 1; letter-spacing: -1px; }
        .total-red { color: #f43f5e; }
        .total-yellow { color: #eab308; }
        .total-green { color: #34d399; }
        .multiplier-row { display: flex; align-items: center; gap: 8px; margin-top: 8px; }
        .mult-badge { background: rgba(234, 179, 8, .1); border: 1px solid rgba(234, 179, 8, .2); border-radius: 6px; padding: 3px 8px; font-size: 11px; font-weight: 700; color: #fbbf24; }
        .ai-box { margin: 0 16px 16px; border: 1px solid rgba(99, 102, 241, .25); border-radius: 12px; background: rgba(99, 102, 241, .06); padding: 14px; }
        .ai-header { display: flex; align-items: center; gap: 6px; margin-bottom: 8px; }
        .ai-icon { font-size: 16px; }
        .ai-title { font-size: 10px; font-weight: 700; letter-spacing: 1.5px; color: #818cf8; }
        .ai-text { font-size: 11px; color: #94a3b8; line-height: 1.6; }
        .ai-warn { color: #fbbf24; font-weight: 600; }
        .clear-btn { width: 100%; margin-top: 10px; padding: 8px; border-radius: 8px; background: rgba(30, 41, 59, .5); border: 1px solid rgba(255, 255, 255, .08); color: #64748b; font-size: 12px; font-weight: 600; font-family: 'Outfit', sans-serif; cursor: pointer; transition: all .2s; }
        .clear-btn:hover { color: #e2e8f0; border-color: rgba(255, 255, 255, .15); }
        .runs-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
        .run-card { background: rgba(30, 41, 59, .35); border: 1px solid rgba(255, 255, 255, .06); border-radius: 12px; padding: 14px 14px 10px; cursor: pointer; transition: all .2s; position: relative; }
        .run-card:hover { border-color: rgba(124, 58, 237, .3); }
        .run-card.selected { border-color: rgba(124, 58, 237, .6); background: rgba(124, 58, 237, .12); box-shadow: 0 0 14px rgba(124, 58, 237, .15); }
        .run-card-label { font-size: 9px; font-weight: 700; letter-spacing: 2px; color: #475569; margin-bottom: 6px; }
        .run-card-value { font-size: 22px; font-weight: 800; color: #e2e8f0; margin-bottom: 2px; }
        .run-card-prob { font-size: 11px; font-weight: 600; }
        .run-card .sel-indicator { position: absolute; top: 10px; right: 10px; width: 8px; height: 8px; border-radius: 50%; background: #7c3aed; display: none; }
        .run-card.selected .sel-indicator { display: block; }
        .run-card-sub { font-size: 10px; color: #475569; margin-top: 4px; }
        .line-tabs { display: flex; gap: 4px; margin-bottom: 10px; }
        .line-tab { flex: 1; padding: 5px 4px; border-radius: 6px; font-size: 10px; font-weight: 600; font-family: 'Outfit', sans-serif; border: 1px solid rgba(255, 255, 255, .07); background: rgba(9, 13, 22, .5); color: #475569; cursor: pointer; text-align: center; transition: all .15s; }
        .line-tab.active-over { background: rgba(16, 185, 129, .15); border-color: rgba(16, 185, 129, .35); color: #34d399; }
        .line-tab.active-under { background: rgba(239, 68, 68, .12); border-color: rgba(239, 68, 68, .3); color: #fca5a5; }
        .prob-bar-track { width: 100%; height: 5px; background: rgba(255, 255, 255, .05); border-radius: 99px; overflow: hidden; margin-top: 6px; }
        .prob-bar-fill { height: 100%; border-radius: 99px; transition: width .4s; }
        .fill-green { background: linear-gradient(to right, #059669, #34d399); }
        .fill-red { background: linear-gradient(to right, #be123c, #f87171); }
        .fill-purple { background: linear-gradient(to right, #7c3aed, #a78bfa); }
        .divider { height: 1px; background: rgba(255, 255, 255, .05); }
        .market-card.active-selection {
            border-color: rgba(16, 185, 129, 0.5) !important;
            background: rgba(16, 185, 129, 0.05) !important;
            box-shadow: 0 0 12px rgba(16, 185, 129, 0.15);
        }
        .market-card.active-selection .market-title::before {
            content: '✓ ';
            color: #34d399;
            font-weight: 900;
        }
        .run-card.selected.active-selection {
            border-color: rgba(16, 185, 129, 0.5) !important;
            background: rgba(16, 185, 129, 0.05) !important;
            box-shadow: 0 0 12px rgba(16, 185, 129, 0.15);
        }
        .run-card.selected.active-selection .run-card-label::before {
            content: '✓ ';
            color: #34d399;
            font-weight: 900;
        }
        
        /* Same Game Parlay (SGP) Ticket Styles */
        .sgp-group {
            ...
            animation: slideInCard 0.25s cubic-bezier(0.16, 1, 0.3, 1);
        }
        .sgp-header {
            display: flex;
        ...
        .sgp-selections-container {
            position: relative;
            padding-left: 20px;
        }
        .sgp-selections-container::before {
            content: '';
            position: absolute;
            left: 6px;
            top: 6px;
            bottom: 6px;
            width: 2px;
            background: var(--sgp-color, #ef4444);
            border-radius: 1px;
        }
        .sgp-selection-item {
            position: relative;
        ...
        .sgp-selection-item:last-child {
            margin-bottom: 0;
        }
        .sgp-selection-item::before {
            content: '';
            position: absolute;
            left: -18px;
            top: 5px;
            ...
            height: 8px;
            border-radius: 50%;
            background: var(--sgp-dot-color, #ef4444);
      ...
      `;
            ...
            window.showPremiumToast = function (message) {
                ...
                toast.style.cssText = `
                    ...
                    box-shadow: 0 4px 20px rgba(0, 0, 0, 0.3), 0 0 15px rgba(16, 185, 129, 0.15);
                    color: #e2e8f0;
                    padding: 12px 20px;
                    border-radius: 12px;
                        ...
                        <div style="display: flex; gap: 8px; margin-bottom: 12px;">
                          <div class="select-wrap" style="flex: 1;">
                            <select id="parlay-saved-match" onchange="loadSavedMatch(this.value)">
                              <option value="">📂 Partidos Guardados...</option>
                              ${sportMatches.map(m => `<option value="${m.id}" ${activeSavedMatch && activeSavedMatch.id === m.id ? 'selected' : ''}>${m.partido} (${m.liga.replace('⚾ MLB — ', '')})</option>`).join('')}
                            </select>
                          </div>
                          ${activeSavedMatch ? `
                            <button onclick="eliminarPartidoGuardado('${activeSavedMatch.id}')" title="Eliminar partido guardado" style="flex-shrink: 0; background: rgba(239, 68, 68, 0.15); border: 1px solid rgba(239, 68, 68, 0.35); color: #fca5a5; padding: 0 12px; border-radius: 10px; cursor: pointer; transition: all 0.2s;" onmouseover="this.style.background='rgba(239, 68, 68, 0.25)'" onmouseout="this.style.background='rgba(239, 68, 68, 0.15)'">
                              🗑️
                            </button>
                          ` : ''}
                        </div>
                    ...
                    if (sportMatches.length === 0) {
                        ...
                        container.innerHTML = `
                            ...
                            <div class="glass-panel flex flex-col items-center justify-center p-6 text-center" style="background: rgba(30, 41, 59, 0.15); border: 1px dashed rgba(255, 255, 255, 0.1); border-radius: 16px; min-height: 160px; display: flex; flex-direction: column; justify-content: center; align-items: center; gap: 10px; margin-top: 10px;">
                                ...
                                <div style="font-weight: 700; color: #f1f5f9; font-size: 13px;">Selecciona un Partido Guardado</div>
                ...
                if (currentSport === 'futbol') {
                    ...
                    container.innerHTML = `
                        ...
                        <div id="saved-match-badge" class="px-3 py-1.5 rounded-lg border border-emerald-500/30 bg-emerald-500/10 text-emerald-400 text-[10px] font-extrabold flex items-center space-x-1.5 mb-3 animate-pulse">
                            <span>📊</span>
                            <span>DATOS REALES DEL MODELO PREDICTIVO ACTIVO</span>
                        </div>

                        <div class="section-label">GANADOR DEL PARTIDO</div>
                        <div class="winner-row" style="margin-bottom:16px;" id="futbol-winner-row">
                          <button class="winner-btn" data-winner="local" onclick="toggleWinnerBtn(this, 'selected-local', 'local', ${activeSavedMatch.datos.ganador.local})">
                            Local (1)<br><span style="font-size:10px;font-weight:400;opacity:.7">${activeSavedMatch.local} · ${Math.round(activeSavedMatch.datos.ganador.local * 100)}%</span>
                          </button>
                          <button class="winner-btn" data-winner="draw" onclick="toggleWinnerBtn(this, 'selected-draw', 'draw', ${activeSavedMatch.datos.ganador.empate})">
                            Empate (X)<br><span style="font-size:10px;font-weight:400;opacity:.7">${Math.round(activeSavedMatch.datos.ganador.empate * 100)}%</span>
                          </button>
                          <button class="winner-btn" data-winner="visita" onclick="toggleWinnerBtn(this, 'selected-away', 'visita', ${activeSavedMatch.datos.ganador.visita})">
                            Visita (2)<br><span style="font-size:10px;font-weight:400;opacity:.7">${activeSavedMatch.visitante} · ${Math.round(activeSavedMatch.datos.ganador.visita * 100)}%</span>
                          </button>
                        </div>

                        <div class="section-label">LÍNEAS DE OVER / UNDER</div>
                        <div class="markets-grid" style="margin-bottom:18px">
                          <div class="market-card" id="futbol-card-goles" onclick="toggleGoalsCard(this)">
                            <div class="market-title">⚽ GOLES TOTALES</div>
                            <select class="market-select" id="futbol-goles-select" onclick="event.stopPropagation()" onchange="changeGoalsSelect(this)">
                              <option value="${activeSavedMatch.datos.goles.over_1_5}">Over 1.5  —  ${Math.round(activeSavedMatch.datos.goles.over_1_5 * 100)}%</option>
                              <option value="${activeSavedMatch.datos.goles.under_1_5}">Under 1.5  —  ${Math.round(activeSavedMatch.datos.goles.under_1_5 * 100)}%</option>
                              <option value="${activeSavedMatch.datos.goles.over_2_5}" selected>Over 2.5  —  ${Math.round(activeSavedMatch.datos.goles.over_2_5 * 100)}%</option>
                              <option value="${activeSavedMatch.datos.goles.under_2_5}">Under 2.5  —  ${Math.round(activeSavedMatch.datos.goles.under_2_5 * 100)}%</option>
                              <option value="${activeSavedMatch.datos.goles.over_3_5}">Over 3.5  —  ${Math.round(activeSavedMatch.datos.goles.over_3_5 * 100)}%</option>
                              <option value="${activeSavedMatch.datos.goles.under_3_5}">Under 3.5  —  ${Math.round(activeSavedMatch.datos.goles.under_3_5 * 100)}%</option>
                            </select>
                          </div>
                          <div class="market-card" id="futbol-card-corners" onclick="toggleCornersCard(this)">
                            <div class="market-title">🚩 TIROS DE ESQUINA</div>
                            <select class="market-select" id="futbol-corners-select" onclick="event.stopPropagation()" onchange="changeCornersSelect(this)">
                              <option value="${activeSavedMatch.datos.corners.over_7_5}">Over 7.5  —  ${Math.round(activeSavedMatch.datos.corners.over_7_5 * 100)}%</option>
                              <option value="${activeSavedMatch.datos.corners.over_8_5}">Over 8.5  —  ${Math.round(activeSavedMatch.datos.corners.over_8_5 * 100)}%</option>
                              <option value="${activeSavedMatch.datos.corners.over_9_5}" selected>Over 9.5  —  ${Math.round(activeSavedMatch.datos.corners.over_9_5 * 100)}%</option>
                              <option value="${activeSavedMatch.datos.corners.over_10_5}">Over 10.5  —  ${Math.round(activeSavedMatch.datos.corners.over_10_5 * 100)}%</option>
                              <option value="${activeSavedMatch.datos.corners.under_9_5}">Under 9.5  —  ${Math.round(activeSavedMatch.datos.corners.under_9_5 * 100)}%</option>
                              <option value="${activeSavedMatch.datos.corners.under_10_5}">Under 10.5  —  ${Math.round(activeSavedMatch.datos.corners.under_10_5 * 100)}%</option>
                            </select>
                          </div>
                        </div>
                    ...
                    container.innerHTML = `
                        ...
                        <div style="margin-bottom: 16px;">
                          <div class="section-label">GANADOR DEL PARTIDO</div>
                          <div class="winner-row" id="beisbol-winner-row">
                            <button class="winner-btn" data-winner="local" onclick="toggleWinnerBeisbol(this, 'local', ${activeSavedMatch.datos.ganador.local})">
                              🏠 <span class="local-name">${activeSavedMatch.local}</span>
                              <span class="prob" style="font-size:11px;font-weight:500;opacity:.7;margin-top:3px;display:block">Local · ${(activeSavedMatch.datos.ganador.local * 100).toFixed(1)}%</span>
                            </button>
                            <button class="winner-btn" data-winner="visita" onclick="toggleWinnerBeisbol(this, 'visita', ${activeSavedMatch.datos.ganador.visita})">
                              ✈️ <span class="visita-name">${activeSavedMatch.visitante}</span>
                              <span class="prob" style="font-size:11px;font-weight:500;opacity:.7;margin-top:3px;display:block">Visita · ${(activeSavedMatch.datos.ganador.visita * 100).toFixed(1)}%</span>
                            </button>
                          </div>
                          <div style="margin-top:10px">
                            <div style="display:flex;justify-content:space-between;font-size:10px;color:#475569;margin-bottom:4px">
                              <span id="beisbol-local-pct">${activeSavedMatch.local} ${(activeSavedMatch.datos.ganador.local * 100).toFixed(1)}%</span>
                              <span id="beisbol-visita-pct">${activeSavedMatch.visitante} ${(activeSavedMatch.datos.ganador.visita * 100).toFixed(1)}%</span>
                            </div>
                            <div style="display:flex;height:6px;border-radius:99px;overflow:hidden;gap:2px">
                              <div id="beisbol-local-bar" style="width:${(activeSavedMatch.datos.ganador.local * 100).toFixed(1)}%;background:linear-gradient(to right,#7c3aed,#a78bfa);border-radius:99px 0 0 99px"></div>
                              <div id="beisbol-visita-bar" style="flex:1;background:linear-gradient(to left,#db2777,#ec4899);border-radius:0 99px 99px 0"></div>
                            </div>
                          </div>
                        </div>
                        ...
                        <div style="margin-bottom:18px">
                          <div class="section-label">TOTAL DE CARRERAS (OVER / UNDER)</div>
                          <div class="runs-grid">
                            <!-- Card 6.5 -->
                            <div class="run-card" data-line="6.5" onclick="toggleRunCard(this)">
                              <div class="sel-indicator"></div>
                              <div class="run-card-label">LÍNEA 6.5</div>
                              <div class="line-tabs">
                                <button class="line-tab" onclick="event.stopPropagation();changeRunTab(this,'over')">Over</button>
                                <button class="line-tab" onclick="event.stopPropagation();changeRunTab(this,'under')">Under</button>
                              </div>
                              <div class="run-card-value">50.0%</div>
                              <div class="run-card-prob" style="color:#34d399">Over 6.5 Carreras</div>
                              <div class="prob-bar-track"><div class="prob-bar-fill fill-green" style="width:50.0%"></div></div>
                              <div class="run-card-sub">Total esperado: ${activeSavedMatch.datos.carreras.total_esperado.toFixed(2)}</div>
                            </div>

                            <!-- Card 7.5 -->
                            <div class="run-card" data-line="7.5" onclick="toggleRunCard(this)">
                              <div class="sel-indicator"></div>
                              <div class="run-card-label">LÍNEA 7.5 ⭐</div>
                              <div class="line-tabs">
                                <button class="line-tab" onclick="event.stopPropagation();changeRunTab(this,'over')">Over</button>
                                <button class="line-tab" onclick="event.stopPropagation();changeRunTab(this,'under')">Under</button>
                              </div>
                              <div class="run-card-value">50.0%</div>
                              <div class="run-card-prob" style="color:#fca5a5">Under 7.5 Carreras</div>
                              <div class="prob-bar-track"><div class="prob-bar-fill fill-red" style="width:50.0%"></div></div>
                              <div class="run-card-sub">Línea más jugada</div>
                            </div>

                            <!-- Card 8.5 -->
                            <div class="run-card" data-line="8.5" onclick="toggleRunCard(this)">
                              <div class="sel-indicator"></div>
                              <div class="run-card-label">LÍNEA 8.5</div>
                              <div class="line-tabs">
                                <button class="line-tab" onclick="event.stopPropagation();changeRunTab(this,'over')">Over</button>
                                <button class="line-tab" onclick="event.stopPropagation();changeRunTab(this,'under')">Under</button>
                              </div>
                              <div class="run-card-value">50.0%</div>
                              <div class="run-card-prob" style="color:#fca5a5">Under 8.5 Carreras</div>
                              <div class="prob-bar-track"><div class="prob-bar-fill fill-red" style="width:50.0%"></div></div>
                              <div class="run-card-sub">Alta probabilidad</div>
                            </div>

                            <!-- Card 9.5 -->
                            <div class="run-card" data-line="9.5" onclick="toggleRunCard(this)">
                              <div class="sel-indicator"></div>
                              <div class="run-card-label">LÍNEA 9.5</div>
                              <div class="line-tabs">
                                <button class="line-tab" onclick="event.stopPropagation();changeRunTab(this,'over')">Over</button>
                                <button class="line-tab" onclick="event.stopPropagation();changeRunTab(this,'under')">Under</button>
                              </div>
                              <div class="run-card-value">50.0%</div>
                              <div class="run-card-prob" style="color:#fca5a5">Under 9.5 Carreras</div>
                              <div class="prob-bar-track"><div class="prob-bar-fill fill-red" style="width:50.0%"></div></div>
                              <div class="run-card-sub">Muy seguro</div>
                            </div>
                          </div>
                        </div>
                        ...
                        return `
                            <div class="sgp-selection-item">
                                <div class="sgp-selection-info">
                                    ...
                                    <div class="sgp-selection-outcome">${pick.mercado}</div>
                                    <div class="sgp-selection-prob" style="color: ${probColor}">Prob: ${probPercent}%</div>
                                </div>
                    ...
                    groupCard.innerHTML = `
                        <div class="sgp-header">
                            <div class="sgp-header-left">
                                ${badgeHtml}
                                <div class="sgp-match-title" title="${partido}">${partido}</div>
                            </div>
                            <div class="sgp-header-right">
                                <div class="sgp-group-prob" title="Cuota combinada del partido">✕ ${groupOdds}x</div>
                                <button class="sgp-group-del" title="Eliminar partido completo" onclick="deleteMatchGroup(\'${partido}\')">✕</button>
                            </div>
                        </div>
                        <div class="sgp-selections-container">
                            ${selectionsHtml}
                        </div>
                        <div class="flex justify-between items-center mt-2 pt-2 border-t border-slate-800/40">
                            <span class="text-[9px] text-slate-500 font-bold uppercase tracking-wider">Monto Apuesta</span>
                            <div class="flex items-center space-x-1">
                                <span class="text-[10px] text-slate-600 font-bold">$</span>
                                <input type="number" 
                                       class="w-16 bg-slate-950/80 border border-slate-800/80 rounded px-1.5 py-0.5 text-right text-[10px] text-emerald-400 font-bold outline-none focus:border-emerald-500" 
                                       value="${window.montoPicks[partido]}" 
                                       min="1" 
                                       step="1" 
                                       onchange="window.actualizarMontoPick('${partido.replace(/'/g, "\\'")}', this.value)" />
                            </div>
                        </div>
                    `;
